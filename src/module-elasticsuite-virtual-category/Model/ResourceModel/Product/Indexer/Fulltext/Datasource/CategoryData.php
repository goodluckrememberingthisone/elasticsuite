...
class CategoryData extends \Smile\ElasticsuiteCatalog\Model\ResourceModel\Product\Indexer\Fulltext\Datasource\CategoryData
{
    ...
    {
        $useDefaultPositionsAttr    = $this->getUseDefaultPositionsAttribute();
        $linkField                  = $this->getEntityMetaData(CategoryInterface::class)->getLinkField();

        $conditions    = [
            "cpi.category_id = use_default_positions.{$linkField}",
            "use_default_positions.store_id = " . $storeId,
            "use_default_positions.attribute_id = " . (int) $useDefaultPositionsAttr->getAttributeId(),
        ];
        ...
        $select = $this->getConnection()->select()
            ...
            ->joinLeft(
                ['use_default_positions' => $useDefaultPositionsAttr->getBackendTable()],
                $joinCondition,
                []
            )
            ->where('cpi.store_id = ?', $storeId)
            ->where('cpi.product_id IN(?)', $productIds)
            ->where(new \Zend_Db_Expr("COALESCE(use_default_positions.value, 1) = 1"))
            ->columns([
    ...
    private function getBaseSelectStore($productIds, $storeId)
    {
        ...
        $select = $this->getConnection()->select()
            ...
            ->where('cpi.store_id = ?', $storeId)
            ->where('cpi.product_id IN(?)', $productIds)
            ->where(new \Zend_Db_Expr("COALESCE(use_default_positions.value, 1) = 0"))
            ->columns([
                'category_id'    => 'cpi.category_id',
                'product_id'     => 'cpi.product_id',
                'is_parent'      => 'cpi.is_parent',
    ...
    private function getVirtualSelectGlobal($productIds, $storeId)
    {
        ...
        $conditions    = [
            "p.category_id = use_default_positions.{$linkField}",
            "use_default_positions.store_id = " . $storeId,
            "use_default_positions.attribute_id = " . (int) $useDefaultPositionsAttr->getAttributeId(),
        ];
        $joinCondition = new \Zend_Db_Expr(implode(" AND ", $conditions));
     ...
     */
    private function getUseDefaultPositionsAttribute()
    {
        $this->useDefaultPositionsAttribute = $this->getEavConfig()
            ->getAttribute(\Magento\Catalog\Model\Category::ENTITY, 'use_default_positions');

        return $this->useDefaultPositionsAttribute;
    }
}
